...
class PdfHelper
{
    ...
    public function header3($x = 0, $y = 0, $title = null, $title_sub = null, $logo_primary = null, $logo_secondary = null, $titleFont = 'times', $ellipseData = null)
    {
        ...
        if ($logo_primary) {
            $this->pdf->image($logo_primary, $centeredX + $leftMargin, $y + 1.5, 21, 17, 'png');
        }
        
        // 固定右側內容的寬度
        $rightContentWidth = 45; // 固定寬度 40mm
        ...
        if ($ellipseData !== null) {
            ...
            // 繪製右側內容（weight 在上面，program 在下面）
            $this->pdf->setFont($titleFont, 'B', 16); // weight 字型
            $this->pdf->SetXY($rightStartX, $startY - 2);
            $this->pdf->Cell($maxWidth, 8, $programText, 0, 1, 'C'); // 水平置中
            
            $this->pdf->setFont($titleFont, 'B', 14); // program 字型
            $this->pdf->SetXY($rightStartX, $startY + 4);
            $this->pdf->Cell($maxWidth, 8, $weightText, 0, 0, 'C'); // 水平置中

            $this->pdf->setFont($titleFont, 'B', 10); // program 字型
            $this->pdf->SetXY($rightStartX, $startY + 10);
            $this->pdf->Cell($maxWidth, 8, $rangeText, 0, 0, 'C'); // 水平置中
        }
    }
