...
class WeightInService{
    ...
    public function schedule($records){
        ...
        $data.='<td rowspan="2" colspan="2">Nation</td>';
        ...
        $data.='<td rowspan="2" style="width:30px;font-size:12px">Out</td>';
        $data.='</tr>';
        $data.='<tr>';
        ...
        foreach($records as $i=>$record){
            $data .='<tr>';
            $data .='<td>'.($i+1).'</td>';
            $data .='<td  width="30"><img src="'.$record["logo"].'" width="30px"/></td>';
            $data .='<td width="60">'.$record["team"].'</td>';
